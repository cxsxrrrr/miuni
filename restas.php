...
try {
	$tipoId = miuni_get_or_create_tipo_id($pdo, 'resta');

	if (isset($_GET['reset'])) {
		miuni_reset_user_exercises($pdo, $userId, $tipoId, 8, 'resta');
...
<body class="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-white p-6" style="background-image:url('assets/games/bgselector.png');background-size:cover;background-position:center;background-repeat:no-repeat;background-attachment:fixed;">
	<main class="max-w-5xl mx-auto">
		<header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-8 text-white">
			<div>
				<h1 class="text-3xl font-extrabold text-emerald-200 drop-shadow-sm">Ejercicios de Resta</h1>
				<p class="mt-2 text-sm font-semibold text-emerald-200 drop-shadow-sm">Selecciona un ejercicio para resolverlo mediante arrastre de números.</p>
			</div>
		...
		<section class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
			<?php foreach ($exercises as $index => $exercise): ?>
				...
				?>
				<a
					href="resta.php?id=<?php echo (int)$exercise['id']; ?>"
